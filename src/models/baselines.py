...
def load_ablation_results(model_type: str) -> dict | None:
    """Read test metrics saved by evaluate.py --model_type for an ablation run.

    File: results/metrics/metrics_test_{model_type}.json
    Generated by:
        python -m src.models.evaluate --model_type {model_type} \\
            --tune_thresholds --window_dir data/processed/windows_no_smote
    """
    ...
    if metrics_path.exists():
        with open(metrics_path) as f:
            return json.load(f)
    log.warning(
        f"  {model_type} metrics not found at {metrics_path.name}. "
        f"Run: python -m src.models.evaluate --model_type {model_type} "
        f"--tune_thresholds --window_dir data/processed/windows_no_smote"
    )
    return None
