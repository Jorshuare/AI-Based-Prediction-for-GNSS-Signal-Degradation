...
\begin{itemize}
  ...
  \item Transformer+LSTM neural network design \& training
  ...
  \item Inference packaging (0.0449\,ms/sample)
...
\begin{poblock}
\textbf{Input}: $(B, 30, 37)$ feature windows

\textbf{Transformer Encoder}\\
2 layers · 8 heads · $d=128$ · $d_{ff}=512$\\
...
\textbf{4 Output Heads}: $+5$\,s, $+15$\,s, $+30$\,s + auxiliary $t{+}0$
\quad (one forward pass, 1.46\,M params)

\textbf{Training}: Focal loss $\gamma=1.0$, class weights $[1,2,5]$,\\
AdamW $10^{-3}$, cosine annealing, early stopping patience\,50
\end{poblock}

\begin{center}
  \includegraphics[width=0.95\columnwidth]{../../report/sentinel_gnss_inside.png}
...
LSTM alone under-recalls (recall 0.645 vs \textbf{0.847}).
...
\begin{tabular}{lcc}
  ...
  SENTINEL calibrated · Trimble & 29.0\,m & 38.7\,\% \\
  \;+ online $\sigma$ scale    & \textbf{26.9\,m} & \textbf{43.2\,\%} \\
...
\begin{center}
  \includegraphics[width=\columnwidth]{../../results/paper_figures/feature_saliency_5s.pdf}
