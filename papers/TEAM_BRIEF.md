...
The goal: give an autonomous vehicle enough warning to switch to backup localisation *before*
GNSS fails, not after. Trained on Beijing + Hong Kong field/public data; evaluated in-domain
...
1. **Problem reformulation — proactive, not reactive.** Every prior GNSS quality method
   (RTKLIB Q-codes, RAIM, recent learning classifiers) reports degradation *as/after it
   happens*. We *forecast* it at 5/15/30 s. To our knowledge this is the first multi-horizon
...
2. **Cross-city generalisation as the deciding metric.** Our strongest, most defensible
   ...
   to 0.15**. Trees memorise city-specific thresholds; the network learns a transferable
...
5. **Hardware-aware feature design.** An explicit `receiver_tier` feature lets one model
   ...
   (Paper 2). (Note: this makes us *hardware-aware*, not "receiver-agnostic" — wording matters.)
...
- **Temporal *order* is a minor factor (E1).** Shuffling the 30 timesteps drops both DL and
...
- **SMOTE doesn't help anyone (E5).** And the reason is *not* distribution distance (KL
...
> "We trained in Beijing + Hong Kong and tested on a city the model never saw (Tokyo). On the
> safety-critical loss-of-fix class, our model held F1 = 0.75 while the strongest classical
> baseline dropped to 0.15. The model learns degradation, it doesn't memorise a city."
...
| Question | Answer |
|----------|--------|
| "Trees beat your model — why publish the network?" | In-domain yes; out-of-domain (Tokyo) the network keeps DEGRADED F1 at 0.75 vs tree's 0.15, runs 10.5× faster, and serves 3 horizons in one model. Deployment needs cross-city robustness. |
| "Does the Transformer actually use time?" | Honestly, temporal *order* contributes ~3% (E1). The benefit is representational transfer, not order modelling. We state this plainly. |
...
| Calibration (E7) | ⚠️ re-run with T=0.40 |
| Lead-time median value | ⏳ read from histogram |
| Tokyo per-class support counts | ⏳ add |
| Per-receiver evaluation (Paper 2) | ⏳ run inference |
| Adaptive EKF + navigation RMSE | ⏳ build |
...
## 10. Paper portfolio — 2 papers + 1 conference (consolidated from 4)

| Output | What it is | Title (working) | Status | Venue |
|--------|-----------|-----------------|--------|-------|
| **Paper A** | Flagship journal — method + multi-horizon + cross-receiver (§6) + cross-city (§7) + EKF | Proactive Multi-Horizon GNSS Degradation Prediction… Cross-City and Cross-Receiver Generalisation | core ✅, receiver ⏳, EKF ⏳ | *GPS Solutions* (Q1) |
| **Paper B** | Benchmark / dataset descriptor (different contribution type) | SENTINEL-GNSS: A Multi-City, Multi-Receiver Benchmark… | documentation ⏳ | *Scientific Data* / *Data in Brief* |
| **Conference** | Cross-city short paper (extended later into Paper A §7) | Does GNSS Degradation Transfer Across Cities? | core result ✅ (E6) | **ION GNSS+ 2026** |

> **Why 2+1, not 4:** the former Papers 2 (cross-receiver) and 3 (cross-city) were thin alone
> and shared the "generalisation" theme. Merged into Paper A as robustness sections they make
> one strong, complete paper and avoid salami-slicing. The cross-city result doubles as the
> conference paper (standard conference→journal extension). **What is "Paper B"?** The
> benchmark/dataset paper — it releases and documents the data, proposes no model, and targets
> a data journal; it does not overlap with Paper A.
>
> **Title fix applied:** never "Receiver-**Agnostic**" — the model uses a `receiver_tier`
> feature, so it is hardware-*aware*. "Cross-Receiver Generalisation" is the accurate framing.
