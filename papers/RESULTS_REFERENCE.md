...
_(Bootstrap CIs = 1,000 iterations, from `evaluate.py`.)_
...
- XGBoost: SMOTE=0.9098, no-SMOTE=0.9193, Δ = −0.0095 (SMOTE _hurts_)
...
both models by ~3.2%. **Interpretation: temporal _ordering_ is a minor contributor for
...
highly discriminative for the in-domain (Beihang) test set.
...
SMOTE's balanced distribution is in fact _nominally closer_ to the test distribution, yet
...
any distributional benefit. The practical takeaway stands — _SMOTE is unnecessary here_ —
...
| Model            | Beihang Macro-F1 | Tokyo Macro-F1 |         Gap | Tokyo CLEAN | Tokyo WARNING | Tokyo DEGRADED |
| ---------------- | ---------------: | -------------: | ----------: | ----------: | ------------: | -------------: |
...
## 10b. Adaptive EKF — Controlled Simulation (✅ CONFIRMED, `results/ekf_demo.json`)

Prediction-informed adaptive EKF (`src/models/adaptive_ekf.py --demo`). Synthetic trajectory
with a GNSS-blockage segment (epochs 120–180); predictor warns from epoch 115.

| Strategy                | Overall RMSE | Blockage-segment RMSE |
| ----------------------- | -----------: | --------------------: |
| GNSS-only               |       25.8 m |                54.4 m |
| Fixed-R EKF             |       21.5 m |                45.6 m |
...
- `feature_prep --extra_horizons 45 60` now adds y_45s/y_60s arrays (✅ backward-compatible,
  verified). Classical baselines can use them immediately; **the DL model still has 3 heads**,
  so DL at +60 s needs a multi-head retrain (Phase 2b — separate tested change).
- Raw per-satellite C/N₀ streams = new extractor + variable-length input + retrain (Phase 3).
- **Do not report extended-horizon or raw-stream numbers until these runs exist.**
