...
a Transformer--BiLSTM neural network for proactive prediction of GNSS signal
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{lX}
    ...
    Architecture & Designed and implemented the Transformer+BiLSTM model (\texttt{transformer\_lstm.py}) \\
    Training & Implemented training loop, Focal Loss, SMOTE integration, model checkpointing (\texttt{train.py}) \\
    Baselines & Implemented BiLSTM-only ablation, RAIM consistency check baseline \\
...
The reasoning: Transformer attention operates on the full 60-epoch window;
...
5--30 seconds in advance from a 60-second feature window?

I chose a \textbf{hybrid Transformer--BiLSTM} after evaluating three candidates:
...
\begin{enumerate}
  ...
  \item \textbf{Transformer+BiLSTM (SENTINEL)}: The Transformer learns global patterns
    (PDOP trends, satellite rise/set across 60\,s); the BiLSTM learns directional
...
\begin{itemize}
  ...
  \item \textbf{Transformer Encoder}: 2 layers, $d_\mathrm{model}=128$, 4 heads,
    $d_\mathrm{ff}=512$, dropout 0.1
  \item \textbf{BiLSTM}: 2 layers, hidden\,=\,256 (128 per direction), dropout 0.1
...
\begin{table}[H]
  ...
  \begin{tabular}{lp{5cm}cc}
    ...
    E1 & Remove Transformer (BiLSTM only) & 0.767 & 0.645 \\
    E2 & Remove BiLSTM (Transformer only) & 0.767 & 0.571 \\
...
Both Transformer-only and BiLSTM-only produce the same Macro-F1 (0.767), but their
...
BiLSTM-only has DEGRADED precision 0.81 but recall only 0.51: it misses many DEGRADED
events.
...
some false positives) while the BiLSTM provides directional confirmation (suppresses
...
\begin{table}[H]
  ...
  \begin{tabular}{lll}
    ...
    BiLSTM hidden           & 256    & Grid: \{128, 256, 512\}; 512 no gain \\
    BiLSTM layers           & 2      & Grid: \{1, 2, 3\}; 3 layers slower \\
...
\begin{itemize}
  ...
  \item Takes a $(60 \times 37)$ normalised numpy array as input
