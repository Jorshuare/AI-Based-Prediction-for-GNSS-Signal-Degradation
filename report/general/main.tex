...
The model processes a 30-second sliding window of 35 receiver-derived features and
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.92\textwidth]{../problem_statememt.png}
...
\textbf{35 features} per epoch in seven groups of five:

\begin{description}
  \item[Position (5)] Latitude, longitude, altitude, and horizontal position standard deviations (\texttt{lat\_std}, \texttt{lon\_std}) derived from the RTKLIB north/east error estimates over the 30-s window.
  \item[Signal strength / C/N$_0$ (5)] Mean, minimum, maximum, standard deviation, and linear trend of C/N$_0$ across all tracked satellites.
  ...
  \item[DOP (5)] PDOP, HDOP, VDOP, GDOP, and horizontal-to-vertical DOP ratio (\texttt{dop\_ratio}).
  \item[Receiver status (5)] Normalised solution quality status, baseline satellite count, solution age, fix continuity fraction, and fix transition count over the window.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.98\textwidth]{../sentinel_gnss_inside.png}
  \caption{SENTINEL-GNSS architecture. A linear projection and positional encoding map the
           30 by 35 input window to dimension 128. A two layer Transformer encoder reads the
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.97\textwidth]{../methodology.png}
  \caption{Processing flow from the GNSS receiver to the planner. Raw RINEX and NMEA become
           35 features in 7 groups, a 30 second sliding window forms a 30 by 35 tensor, the
           model produces the three horizon forecasts, and the planner consumes them.}
  \label{fig:method}
\end{figure}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{lX}
    ...
    \textbf{Ogunlade Joshua} (LS2525237) &
      ...
      pipeline (\code{run\_rtklib.py}); 35-feature implementation with all
...
\begin{longtable}{cllp{4.5cm}}
  \caption{All 35 GNSS features used as SENTINEL model inputs.} \\
  \toprule
  \textbf{\#} & \textbf{Feature} & \textbf{Group} & \textbf{Description} \\
  \midrule
  \endhead
  1  & \code{lat}                   & Position              & Latitude from RTKLIB solution \\
  2  & \code{lon}                   & Position              & Longitude from RTKLIB solution \\
  3  & \code{alt}                   & Position              & Altitude above ellipsoid (m) \\
  4  & \code{lat\_std}              & Position              & Mean north position std dev over window (\code{sdn}) \\
  5  & \code{lon\_std}              & Position              & Mean east position std dev over window (\code{sde}) \\
  6  & \code{mean\_cnr}             & Signal strength / C/N$_0$ & Mean C/N$_0$ across all tracked satellites \\
  7  & \code{min\_cnr}              & Signal strength / C/N$_0$ & Minimum C/N$_0$ \\
  8  & \code{max\_cnr}              & Signal strength / C/N$_0$ & Maximum C/N$_0$ \\
  9  & \code{std\_cnr}              & Signal strength / C/N$_0$ & Standard deviation of C/N$_0$ \\
  10 & \code{cnr\_trend}            & Signal strength / C/N$_0$ & Linear slope of mean C/N$_0$ over 30-s window (dB-Hz/s) \\
  11 & \code{num\_satellites}       & Satellite count       & Number of satellites used in current SPP solution \\
  12 & \code{sat\_mean}             & Satellite count       & Mean satellite count over 30-s window \\
  13 & \code{sat\_min}              & Satellite count       & Minimum satellite count over 30-s window \\
  14 & \code{sat\_visibility}       & Satellite count       & Ratio of used satellites to estimated maximum \\
  15 & \code{sat\_drop\_rate}       & Satellite count       & Mean of negative satellite-count changes over window \\
  16 & \code{pdop}                  & DOP                   & Position Dilution of Precision (3D) \\
  17 & \code{hdop}                  & DOP                   & Horizontal DOP \\
  18 & \code{vdop}                  & DOP                   & Vertical DOP \\
  19 & \code{gdop}                  & DOP                   & Geometric DOP \\
  20 & \code{dop\_ratio}            & DOP                   & Horizontal-to-vertical DOP ratio (hdop / vdop) \\
  21 & \code{solution\_status}      & Receiver status       & Normalised solution quality (1=Fix, 0=Single SPP) \\
  22 & \code{baseline\_sats}        & Receiver status       & Number of satellites in baseline solution \\
  23 & \code{solution\_age}         & Receiver status       & Age of differential solution (s) \\
  24 & \code{fix\_continuity}       & Receiver status       & Fraction of window epochs with Fix-quality solution \\
  25 & \code{fix\_transitions}      & Receiver status       & Number of solution-quality transitions in window \\
  26 & \code{position\_variance}    & Temporal patterns     & Variance of position error over 30-s window \\
  27 & \code{cnr\_variance}         & Temporal patterns     & Variance of mean C/N$_0$ over 30-s window \\
  28 & \code{elevation\_violations} & Temporal patterns     & Fraction of epochs with low-elevation satellites \\
  29 & \code{multipath}             & Temporal patterns     & Multipath proxy derived from position-C/N$_0$ discrepancy \\
  30 & \code{clock\_bias}           & Temporal patterns     & Mean receiver clock bias over window \\
  31 & \code{iono\_delay}           & Atmospheric effects   & Ionospheric delay estimate (Klobuchar model) \\
  32 & \code{tropo\_delay}          & Atmospheric effects   & Tropospheric delay estimate (Saastamoinen model) \\
  33 & \code{cycle\_slips}          & Atmospheric effects   & Cycle slip count over 30-s window \\
  34 & \code{residual\_mean}        & Atmospheric effects   & Mean pseudorange residual \\
  35 & \code{residual\_std}         & Atmospheric effects   & Standard deviation of pseudorange residuals \\
  \bottomrule
\end{longtable}
