...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\color{buaaBlue}\textbf{SENTINEL-GNSS} — Team Report}
\fancyhead[R]{\small\color{buaaNavy}Beihang University H3I · 2025}
...
\begin{titlepage}
  \centering
  \vspace*{1cm}
  {\color{buaaNavy}\rule{\linewidth}{2pt}}\\[0.4cm]
  {\huge\bfseries\color{buaaNavy} SENTINEL-GNSS}\\[0.3cm]
  {\Large\color{buaaBlue} Proactive Multi-Horizon Prediction of GNSS Signal\\[0.2cm]
  Degradation for Autonomous Vehicle Navigation}\\[0.4cm]
  {\color{buaaNavy}\rule{\linewidth}{2pt}}\\[1.5cm]

  {\large\bfseries Team Pilot Project — General Report}\\[0.5cm]
  ...
  {\large School of Electronics and Information Engineering (H3I)}\\[0.3cm]
  {\large Beihang University (BUAA)}\\[0.3cm]
  {\large Beijing, China · 2025}\\[2cm]

  \begin{minipage}{0.85\textwidth}
    \centering
    \itshape
    Submitted in partial fulfilment of the requirements for the \\
    Pilot Project module, H3I International School, Beihang University.
  \end{minipage}
\end{titlepage}
...
Existing monitoring approaches—RTKLIB quality codes, RAIM, and recent machine-learning
classifiers—are \emph{reactive}: they report degradation only after it has affected
positioning performance, giving the vehicle zero preparation time.
...
The model processes a 60-second sliding window of 37 receiver-derived features and
...
measurement noise covariance inflation, producing a 48.8\,\% reduction in positioning
RMSE during degraded epochs on the Trimble Shinjuku dataset.
...
A live, multilingual dashboard (5 languages) visualises all fusion tracks in real time.
\end{abstract}
...
\begin{enumerate}
  ...
  \item \textbf{SENTINEL-GNSS architecture.} A unified Transformer--BiLSTM network with
    three output heads ($+5$\,s, $+15$\,s, $+30$\,s) achieves Macro-F1\,=\,0.821 at the
    +5\,s horizon using 1.46\,M parameters and runs at 0.039\,ms per sample.

  \item \textbf{Cross-city generalisation.} Training on Beihang + Hong Kong and evaluating
    on fully held-out Tokyo, the network maintains DEGRADED-class F1\,=\,0.75 while the
    strongest classical baseline (RandomForest) drops to 0.15.
...
\textbf{37 features} per epoch in seven groups:

\begin{description}
  \item[Position quality (4)] Horizontal/vertical position error, PDOP, HDOP.
  \item[Signal strength (8)] Mean C/N$_0$, per-satellite C/N$_0$ statistics, variability.
  \item[Satellite geometry (6)] Satellite count, elevation angle statistics, GDOP.
  \item[Pseudorange (7)] Residuals, delta-pseudorange, multipath indicators.
  \item[Temporal patterns (5)] Rolling mean/std of C/N$_0$ and error over 10-s window.
  \item[Atmospheric (4)] Tropospheric and ionospheric delay estimates.
  \item[Receiver status (3)] \texttt{receiver\_tier} (0 professional to 3 consumer),
    lock time, cycle-slip count.
\end{description}

The \texttt{receiver\_tier} feature is critical: it lets one model reconcile identical
C/N$_0$ readings that mean different quality levels on different hardware.
Features are $z$-score normalised using training-set statistics only.

\subsection{Sliding Window Construction}

A causal sliding window of $W = 60$ epochs (60\,s at 1\,Hz) produces input tensors
of shape $(60 \times 37)$.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{../../results/paper_figures/fig10_dataset_composition.pdf}
...
SENTINEL-GNSS is a hybrid Transformer--BiLSTM network designed to exploit two
complementary temporal scales:

\begin{itemize}
  \item \textbf{Global dependencies (Transformer).} Satellite rising/setting events and
    PDOP trends that recur across a 60-second window are captured by multi-head self-attention
    \cite{vaswani2017attention}, which can directly relate distant timesteps without
    gradient decay.

  \item \textbf{Sequential dynamics (BiLSTM).} The directional approach to a degradation
    event (C/N$_0$ gradually declining, pseudorange residuals growing) is captured by a
    bidirectional LSTM \cite{hochreiter1997lstm} which maintains a hidden state summarising
...
\begin{equation}
  ...
  \mathbf{h}^\mathrm{T}_{T} \xrightarrow{\text{BiLSTM}}
  \mathbf{h}^\mathrm{L}_T \xrightarrow{\text{FC}_h}
  \hat{y}^{(h)},\quad h \in \{5,15,30\}
\end{equation}

where the Transformer encoder has $d_\mathrm{model}=128$, 4 heads, 2 layers,
feedforward dimension $d_\mathrm{ff}=512$, dropout 0.1;
and the BiLSTM has hidden dimension 256, 2 layers.
A shared auxiliary head over the BiLSTM output regularises against dataset shift.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.78\textwidth]{../../results/paper_figures/fig14_architecture.pdf}
  \caption{SENTINEL-GNSS architecture. The Transformer encoder captures global
           temporal patterns; the BiLSTM captures sequential approach dynamics.
           Three output heads share one forward pass.}
  \label{fig:arch}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\textwidth]{../../results/figures/learning_curves.pdf}
  \caption{Training and validation loss/F1 curves. Validation F1 stabilises by epoch 30;
           early stopping terminates at epoch 65.}
  \label{fig:learning}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{../../results/figures/multi_horizon_comparison.pdf}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{../../results/figures/confusion_matrices_test.pdf}
...
Table~\ref{tab:ablation} confirms that both the Transformer and the BiLSTM contribute
to performance.

\begin{table}[H]
  \centering
  \caption{Ablation results at the $+5$\,s horizon. The full Transformer+BiLSTM
  ...
  \begin{tabular}{lcccc}
    ...
    BiLSTM-only      & 1.03\,M & 0.767 & 0.702 & 0.645 \\
    \textbf{Full (Trans+BiLSTM)} & \textbf{1.46\,M} &
      \textbf{0.821} & \textbf{0.773} & \textbf{0.718} \\
    \bottomrule
  \end{tabular}
\end{table}

The Transformer alone over-flags DEGRADED (precision 0.42); the BiLSTM provides
directional sequential context that suppresses false alarms.
The BiLSTM alone under-detects DEGRADED (recall 0.645 vs 0.847); the Transformer
...
\begin{equation}
  ...
  \end{cases}
\end{equation}
where $r_\mathrm{base} \in \{4\,\mathrm{m},\, 8\,\mathrm{m}\}$ for Trimble/u-blox
and $r_\mathrm{deg} = 40$\,m.
This \emph{pre-emptive} inflation begins 3--5 epochs before the worst GPS measurements
arrive, because SENTINEL's predictions arrive early.
...
SENTINEL runs at 0.039\,ms per sample, making it 10.5$\times$ faster than three
...
In-domain (Beihang + HK test set), the Random Forest achieves Macro-F1\,=\,0.924
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{lX}
    ...
    \textbf{Ojerinde Joel} (LS2525253) &
      ...
      (\code{feature\_list.py}); SENTINEL Transformer--BiLSTM design, training,
      ...
      analysis); inference pipeline (0.039\,ms); production dashboard
...
navigation safety.
The Transformer--BiLSTM architecture learns transferable temporal signatures of
...
\begin{longtable}{cllp{4.5cm}}
  \caption{All 37 GNSS features used as SENTINEL model inputs.} \\
  \toprule
  \textbf{\#} & \textbf{Feature} & \textbf{Group} & \textbf{Description} \\
  \midrule
  \endhead
  1  & \code{pdop}              & Position quality & Dilution of precision (3D) \\
  2  & \code{hdop}              & Position quality & Horizontal DOP \\
  3  & \code{vdop}              & Position quality & Vertical DOP \\
  4  & \code{pos\_error\_m}     & Position quality & RTK error (HK only; NaN Beihang) \\
  5  & \code{cn0\_mean}         & Signal strength  & Mean C/N$_0$ across SVs \\
  6  & \code{cn0\_min}          & Signal strength  & Minimum C/N$_0$ \\
  7  & \code{cn0\_max}          & Signal strength  & Maximum C/N$_0$ \\
  8  & \code{cn0\_std}          & Signal strength  & Std of C/N$_0$ \\
  9  & \code{cn0\_below\_30}    & Signal strength  & Fraction of SVs with C/N$_0 < 30$\,dB-Hz \\
  10 & \code{cn0\_below\_35}    & Signal strength  & Fraction of SVs with C/N$_0 < 35$\,dB-Hz \\
  11 & \code{cn0\_spread}       & Signal strength  & C/N$_0$ range (max $-$ min) \\
  12 & \code{cn0\_q1}           & Signal strength  & 25th percentile of C/N$_0$ \\
  13 & \code{n\_sv}             & Geometry         & SVs used in SPP solution \\
  14 & \code{n\_sv\_total}      & Geometry         & Total tracked SVs \\
  15 & \code{el\_mean}          & Geometry         & Mean elevation angle (degrees) \\
  16 & \code{el\_min}           & Geometry         & Minimum elevation \\
  17 & \code{gdop}              & Geometry         & Geometric DOP \\
  18 & \code{az\_std}           & Geometry         & Std of azimuth angles (sky coverage) \\
  19 & \code{prange\_res\_rms}  & Pseudorange      & RMS pseudorange residual \\
  20 & \code{prange\_res\_max}  & Pseudorange      & Max absolute residual \\
  21 & \code{prange\_res\_std}  & Pseudorange      & Std of residuals \\
  22 & \code{delta\_prange}     & Pseudorange      & Epoch-to-epoch pseudorange change \\
  23 & \code{multipath\_idx}    & Pseudorange      & $(|\bar{r}| - \sigma_\mathrm{noise})/\sigma_\mathrm{noise}$ \\
  24 & \code{lock\_time\_min}   & Pseudorange      & Minimum satellite lock time (s) \\
  25 & \code{cycle\_slip\_cnt}  & Pseudorange      & Cycle slips this epoch \\
  26 & \code{cn0\_mean\_10s}    & Temporal         & 10\,s rolling mean of \code{cn0\_mean} \\
  27 & \code{cn0\_delta\_10s}   & Temporal         & C/N$_0$ trend over 10\,s \\
  28 & \code{pdop\_ma10}        & Temporal         & 10\,s rolling mean of PDOP \\
  29 & \code{pdop\_trend}       & Temporal         & PDOP trend over 10\,s \\
  30 & \code{err\_trend}        & Temporal         & Positioning error trend (HK only) \\
  31 & \code{tropo\_zenith}     & Atmospheric      & Tropospheric zenith delay (Saastamoinen) \\
  32 & \code{iono\_tec}         & Atmospheric      & Ionospheric TEC (Klobuchar) \\
  33 & \code{tropo\_grad\_n}    & Atmospheric      & Tropospheric north gradient \\
  34 & \code{tropo\_grad\_e}    & Atmospheric      & Tropospheric east gradient \\
  35 & \code{receiver\_tier}    & Receiver status  & Hardware class (0=Septentrio, 3=consumer) \\
  36 & \code{lock\_time\_avg}   & Receiver status  & Mean satellite lock time \\
  37 & \code{cycle\_slip\_rate} & Receiver status  & Cycle slips per second (30\,s window) \\
  \bottomrule
\end{longtable}
...
\begin{table}[H]
  ...
  \begin{tabular}{lp{5.5cm}ccc}
    ...
    E1 & Transformer-only (no BiLSTM) & 0.767 & 0.725 & 0.571 \\
    E2 & BiLSTM-only (no Transformer) & 0.767 & 0.702 & 0.645 \\
