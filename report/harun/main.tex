...
each evaluated via 5-fold CV, for 1,080 training runs in total.
The configuration with the highest validation Macro-F1 was selected.
...
\begin{table}[H]
  ...
  \begin{tabular}{lcccc}
    ...
    Random Forest         & 0.910 & 0.916 & 0.789 & 0.12 \\
    XGBoost               & 0.919 & 0.926 & 0.810 & 0.23 \\
    \midrule
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.86\textwidth]{../../results/paper_figures/fig03_model_comparison.pdf}
  \caption{Baseline comparison on the held-out in-domain test set (Macro-F1 at the
           $+5$\,s horizon). The classical and ablation baselines I implemented are shown
           against the full SENTINEL model. The tree baselines lead in-domain, a result we
           report openly; the cross-city test in the next section reverses this ranking.}
...
\begin{figure}[H]
  ...
  \caption{Cross-city DEGRADED-class F1 on unseen Tokyo. The Random Forest I evaluated
           collapses to 0.15 while SENTINEL holds 0.75. This is the central evidence in our
           argument that in-domain accuracy is the wrong metric for cross-city deployment.}
  \label{fig:crosscityrf}
\end{figure}
...
\begin{enumerate}
  ...
  \item SENTINEL architecture: Transformer+LSTM, 1.46M parameters, 0.0449ms
  \item Cross-city generalisation: 0.753 vs 0.148 Tokyo DEGRADED F1
