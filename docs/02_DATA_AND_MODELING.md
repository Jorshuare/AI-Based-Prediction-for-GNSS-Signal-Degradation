...
**What this document is:** the authoritative story of *the science* — how the data was collected,
...
Hard because: (a) it depends on *temporal trends*, not a single instant; (b) one model must predict
...
*Why predict instead of detect?* Detection tells you the signal is already gone. **Prediction gives
...
### 2.1 Our own field data (Beihang, Beijing)
...
*Justification:* the scenarios deliberately span sudden vs gradual and full vs partial loss, so the
model learns *transitions*, not a single environment. Scenario A is the one surfaced in the
...
*Justification for UrbanNav Tokyo as the held-out city:* unseen city, unseen receivers, different
...
| Temporal trends | pdop_delta, hdop_delta | *Rate of change* — the key to forecasting |
...
*Why hand-crafted, not raw?* GNSS-domain features encode physics (a falling C/N₀ trend is
*meaningful*), giving a smaller, more generalisable model than feeding raw pseudoranges. *Why 37 and
not 100?* Each is justified by GNSS knowledge; redundant features were dropped. The **trend**
features (cnr_trend, sat_drop_rate, *_delta) are what make prediction (not just detection) possible.

**Windowing:** 30-second sliding windows (e.g. 30 epochs at 1 Hz). *Why 30 s?* Long enough to
...
- **Classical baselines (RF/XGBoost):** **SMOTE** is applied, because tree models have no focal-loss
  ...
  distribution is actually *closer* to the test distribution than the raw one — so SMOTE is the
  fairer choice for the baselines.

*Justification:* match the imbalance remedy to the model family; report both honestly.
...
**Transformer encoder** — self-attention sees relationships between *any* two time steps (e.g. a
multipath signature 25 s ago and now):
$$\text{Attention}(Q,K,V)=\text{softmax}\!\left(\frac{QK^\top}{\sqrt{d_k}}\right)V$$
*Why 2 layers / 8 heads / d=128 / d_ff=512?* enough capacity for the patterns without overfitting a
modest dataset; multiple heads attend to different cues (geometry vs strength).

**Bidirectional LSTM** — captures *directional momentum* toward failure (is the signal trending
down?). The LSTM cell’s gated memory avoids the vanishing-gradient problem over the 30-step window.
*Why both Transformer and LSTM?* attention finds patterns; the LSTM reasons about *causal trend*.
...
label smoothing. *Why AdamW?* decoupled weight decay generalises better than Adam/SGD here and is
stable for transformers.

**Focal loss:** with $\gamma=1.0$ and class weights $\alpha=[1,2,5]$,
$$\text{FL}(p_t) = -\,\alpha_t\,(1-p_t)^{\gamma}\,\log(p_t).$$
*Why γ=1.0?* a moderate focusing strength — enough to prioritise the rare DEGRADED class without
...
| **ECE** (calibration) | Gap between predicted confidence and reality | The EKF *uses* P(DEGRADED), so it must be trustworthy |
...
*Reading it (the key finding):* **on an unseen city, RandomForest collapses on the safety-critical
DEGRADED class (0.148), while the deep model keeps its DEGRADED skill (0.75) and even improves it.**
The deep model learned *generalisable degradation physics*; the trees memorised Beijing. Combining
...
time the state doesn’t change in 5 s.** *So why is the model valuable?* Its worth is in the **~6 % of
...
**cross-city** (0.892 vs 0.886 vs ≤0.821). *Why soft-vote?* simpler, and it preserves DL’s DEGRADED
...
1. **Prediction-driven adaptive R:** $R(t) = r_{base} + (r_{deg}-r_{base})\,P(\text{DEGRADED}\,|\,t)$
   — distrust GNSS *before* a predicted blockage (a 5 s pre-emptive head-start).
...
*Honest nuance:* the **aiding** (odometry/NHC/ZUPT) is the big, robust win. Prediction-driven
**adaptive-R** helps a weak GNSS-only platform in severe multipath but is *counter-productive* on a
...
realistic range, because GNSS is the only absolute *heading* reference and discarding it lets the
...
- **Transformer + BiLSTM hybrid** → pattern *and* causal-trend reasoning (ablation-confirmed).
